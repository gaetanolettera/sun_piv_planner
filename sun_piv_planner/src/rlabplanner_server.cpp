...
class RLabplannerAction
{
protected:

  ros::NodeHandle nh_;
  actionlib::SimpleActionServer<sun_plan_msgs::RLabplannerAction> as_; // NodeHandle instance must be created before this line.
...
public:
  ...
  RLabplannerAction(std::string name) :
    as_(nh_, name, boost::bind(&RLabplannerAction::executePlanning, this, _1), false),
    action_name_(name)
  {
    as_.start();
  }

  ~RLabplannerAction(void)
  {
  }
  ...
  void executePlanning(const sun_plan_msgs::RLabplannerGoalConstPtr &goal)
  {
    // helper variables
    ros::Rate r(1000);
    bool success = true;
			...
			if(ik_found)
			{
				...
				if (gp_ik_found == true)
				{
			  		...
			  		if(feedback_.planning_pivoting.val == 1)
					{
						...
						ROS_INFO("PIVOTING execution completed!");
...
{
  ros::init(argc, argv, "rlabplanner_server");

  RLabplannerAction rlabplanner("rlabplanner");
  ros::spin();
